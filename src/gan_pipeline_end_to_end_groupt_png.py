...
def sequence_t_indexes(start_t=0, end_t=200, interval=40, group=10):
    sequence = []
...
def generate_q_sequence_samples(
    ...
    save_indices = sequence_t_indexes(start_t=args.start_t, end_t=args.end_t, interval=args.interval, group=args.group)[::-1]
            ...
            for t in save_indices:
                ...
                for i, img in enumerate(images):
                    ...
                    if index >= num_samples:
                        return
                    Image.fromarray(img).save(os.path.join(save_dir, 'real', f'{t:04d}', f'{index:05d}.png'))
            ...
            for t in indices:
                ...
                if t in save_indices:
                    ...
                    for i, img in enumerate(images):
                        ...
                        if index >= num_samples:
                            return
                        Image.fromarray(img).save(os.path.join(save_dir, 'fake', f'{t:04d}', f'{index:05d}.png'))
            total += global_batch_size
...
def main(args):
    ...
    for iter in range(args.iteration_num):
        ...
        real_sampler, real_loader = get_sampler_and_loader(args, os.path.join(sample_dir,  "real"), rank, logger)
        fake_sampler, fake_loader = get_sampler_and_loader(args, os.path.join(sample_dir,  "fake"), rank, logger)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment_name", type=str, default="gan_pipeline_end_to_end_groupt_png")
    ...
    parser.add_argument("--image_size", type=int, choices=[32, 128, 224, 256, 512], default=256)
